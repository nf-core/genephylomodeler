# nf-core/phyloanalysis: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/phyloanalysis/usage](https://nf-co.re/phyloanalysis/usage)
...
```bash
nextflow run nf-core/phyloanalysis --input ./samplesheet.csv --outdir ./results -profile docker
```
...
```bash
nextflow run nf-core/phyloanalysis -profile docker -params-file params.yaml
```
...
```bash
nextflow pull nf-core/phyloanalysis
```
...
First, go to the [nf-core/phyloanalysis releases page](https://github.com/nf-core/phyloanalysis/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
